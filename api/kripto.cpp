...
int changeBalanse(int userBuy_id, int userSell_id, int lot_pair, float quantity, float price) {
    int lot1, lot2;
    getPair(lot_pair, lot1, lot2);
    vector<vector<string>> abba;
    getu(abba);
    for (int i = 0; i < abba.size(); i++) {
        if(stoi(abba[i][1]) == userBuy_id && stoi(abba[i][2]) == lot1) {
            float q = stof(abba[i][3]);
            q += (quantity);
            abba[i][3] = to_string(q);
        } else if(stoi(abba[i][1]) == userSell_id && stoi(abba[i][2]) == lot1) {
            float q = stof(abba[i][3]);
            q -= (quantity);
            abba[i][3] = to_string(q);
        } else if(stoi(abba[i][1]) == userBuy_id && stoi(abba[i][2]) == lot2) {
            float q = stof(abba[i][3]);
            q -= (quantity * price);
            abba[i][3] = to_string(q);
        } else if(stoi(abba[i][1]) == userSell_id && stoi(abba[i][2]) == lot2) {
            float q = stof(abba[i][3]);
            q += (quantity * price);
            abba[i][3] = to_string(q);
        }
    ...
    for (int i = 0; i < abba.size(); i++) {
        if (stoi(abba[i][1]) == userSell_id || stoi(abba[i][1]) == userBuy_id) {
            ab.push_back(abba[i]);
        }
    }
    string str = "";
    code(str, "DELETE FROM user_lot WHERE user_lot.user_id = '" + to_string(userBuy_id) + "'");
    code(str, "DELETE FROM user_lot WHERE user_lot.user_id = '" + to_string(userSell_id) + "'");
    for (int i = 0; i < ab.size(); i++) {
        code(str, "INSERT INTO user_lot VALUES ('" + ab[i][1] + "', '" + ab[i][2] + "', '" + ab[i][3] + "')");
    }
...
int DeleteOrder(string& str, string http) {
    ...
    if (order_for_delete != -1) {
        ...
        code(str, "DELETE FROM order WHERE order.order_id = '" + order_id_str + "'");
    } 
...
int kripto(string& str, string http) {
    mtx.lock();
